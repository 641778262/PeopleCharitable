...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	long REFRESH_LOADING_MIN_TIME = 2000;// ms
	...
	public final void onRefreshComplete() {
		...
		if (state != PULL_TO_REFRESH) {// FIXME
			...
			// 刷新时间2s
			if (complete_loading_time - start_loading_time <= REFRESH_LOADING_MIN_TIME) {

				handler.postDelayed(refreshCompleteTask,
						REFRESH_LOADING_MIN_TIME);
			} else {
				resetHeader();
			}
		}
	...
	public void resetRefreshTime() {
		complete_loading_time = -1;
		start_loading_time = -1;
	}
			...
			if (isBeingDragged) {
				...
				if (state == RELEASE_TO_REFRESH && null != onRefreshListener) {
					...
					onRefreshListener.onRefresh();
						...
						&& (lastMotionY - initialMotionY) / FRICTION > headerHeight / 2) {
					setRefreshingInternal(true);
				} else {
					smoothScrollTo(0);
				}
		...
		case MotionEvent.ACTION_MOVE: {
			if (isReadyForPull()) {// FIXME
				...
				if (yDiff > touchSlop && yDiff > xDiff) {
					...
					// if ((mode == MODE_PULL_DOWN_TO_REFRESH || mode ==
					// MODE_BOTH)
					...
					// // }
					// }
					// } else if ((mode == MODE_PULL_UP_TO_REFRESH || mode ==
					// MODE_BOTH)
					...
					// }
					// }
					boolean b1 = (mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH)
							&& dy >= 0.0001f && isReadyForPullDown();
					boolean b2 = (mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH)
							&& dy <= 0.0001f
							&& isReadyForPullDown()
							&& getScrollY() < 0;
	...
	protected final LoadingLayoutBoth getFooterLayout() {
		return footerLayout;
	}

	protected final LoadingLayoutBoth getHeaderLayout() {
		return headerLayout;
	}
	...
	private void init(Context context, AttributeSet attrs) {
		...
		String pullLabel = context
				.getString(R.string.pull_to_refresh_pull_label_list);
		String refreshingLabel = context
				.getString(R.string.pull_to_refresh_refreshing_label_list);
		String releaseLabel = context
				.getString(R.string.pull_to_refresh_release_label_list);

		// footlayout View Strings
		String pullMoreLabel = context
				.getString(R.string.pull_to_more_pull_label);
		String moreRefreshingLabel = context
				.getString(R.string.pull_to_more_refreshing_label);
		String moreReleaseLabel = context
				.getString(R.string.pull_to_more_release_label);

		// Add Loading Views
		if (mode == MODE_PULL_DOWN_TO_REFRESH || mode == MODE_BOTH) {
			headerLayout = new LoadingLayoutBoth(context,
		...
		if (mode == MODE_PULL_UP_TO_REFRESH || mode == MODE_BOTH) {
			footerLayout = new LoadingLayoutBoth(context,
					MODE_PULL_UP_TO_REFRESH, moreReleaseLabel, pullMoreLabel,
					moreRefreshingLabel);
	...
	private boolean pullEvent() {
		...
		case MODE_PULL_DOWN_TO_REFRESH:
		default:
			if (lastMotionY < initialMotionY) {
				if (isRefreshing()) {
					newHeight = -(headerHeight + Math.round(Math.min(
							lastMotionY - initialMotionY, 0) / FRICTION));//
				} else {
					newHeight = Math.round(Math.min(initialMotionY
							- lastMotionY, 0)
							/ FRICTION);// FIXME
				}
			} else {
				newHeight = Math.round(Math
						.min(initialMotionY - lastMotionY, 0) / FRICTION);// FIXME
			}
...
//					headerLayout.releaseToRefresh(newHeight, headerHeight);
//				}
//			}
	...
	public long getRefreshCompleteTime() {
		return complete_loading_time;
	}

	public long getStartRefreshTime() {
		return start_loading_time;
	}
}
